...
if (sxtResponse.error) {
  console.error("SXT API Request Error:", sxtResponse.response ? sxtResponse.response.data : sxtResponse);
  throw Error(`SXT API request failed: ${sxtResponse.response ? JSON.stringify(sxtResponse.response.data) : 'Network Error or Timeout'}`);
}
